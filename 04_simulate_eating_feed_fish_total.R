## Scenario sim - nFIFO - FMFO data eating edible wild fish
## nFIFO as fraction of feed replacement

fracs<-seq(0, 1, by = 0.01)
output<-numeric()
nut_output<-numeric()
fo_output<-numeric()
...
for(i in 1:length(fracs)){
    ...
    mutate(total_fishoil = nor_wild_use_from_fo,
           ...
           new_trimmings = species_fishoil_edible - edible_seafood,
    ...
    # save new trimmings from edible feed fish
    new_trimmings_from_feed<-sum(whole_edible$new_trimmings)
    ...
    nfifo$nfifo<-nfifo$salmon_nut_yield / nfifo$feed_nut_yield
    
    ## Outputs: nFIFO, seafood produced, salmon produced, FO needed
    nuts<-nfifo %>% mutate(frac = fracs[i])
    
    fo<-data.frame(fo_missing = missing_feed, 
                   fo_missing_prop = missing_feed_prop, frac = fracs[i],
                   new_trimmings = new_trimmings_from_feed)
